...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table" role="presentation">
		...
		<tr>
			...
			<?php
			switch_to_blog( $id );
			wp_dropdown_roles( retraceur_get_default_role() );
			restore_current_blog();
			?>
			</select></td>
		</tr>
...
if ( current_user_can( 'create_users' ) && apply_filters( 'show_network_site_users_add_new_form', true ) ) :
	?>
<h2 id="add-new-user"><?php _e( 'Add Contributor' ); ?></h2>
<form action="<?php echo esc_url( network_admin_url( 'site-users.php?action=newuser' ) ); ?>" id="newuser" method="post">
	...
	<?php submit_button( __( 'Add Contributor' ), 'primary', 'add-user', true, array( 'id' => 'submit-add-user' ) ); ?>
</form>
<?php endif; ?>
</div>
<?php
